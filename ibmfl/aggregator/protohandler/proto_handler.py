...
class ProtoHandler(abc.ABC):
    ...
    def query_parties(self, payload, lst_parties, perc_quorum=1.,
        ...
        if not collect_metrics:
            metrics_party = None
        if fusion_state is not None and MessageType.TRAIN_BY_TIER    == msg_type:
    ...
    def get_party_preferences(self, party_ids, data_queries):
        '''
        Get party preferences for each party in the list of party_ids
        :param party_ids: List of party ids
        :type party_ids: `list`
        :param data_queries: List of data queries
        :type data_queries: `list`
        :return: List of party preferences
        :rtype: `list`
        '''
        try:
            response = self.query_parties(data_queries, party_ids,
                      msg_type=MessageType.REQUEST_TIER_PREFERENCES,
                      return_responding_parties=True
                      )
        except Exception as fl_ex:
            logger.exception(
                'Error occurred while sending request to parties' + str(fl_ex))
                ...
                if self.max_timeout:
                    if (round(time.time()-start) >= self.max_timeout):
                        if not self.has_reached_quorum(party_ids, id_request,
                                                perc_quorum=perc_quorum,
                                                quorum_offset=quorum_offset):
                            logger.error('One or more party did not reply in time.')
                            target_quorum = int(math.ceil((len(party_ids) + quorum_offset) * perc_quorum))
...
class ProtoHandlerRabbitMQ(ProtoHandler):
    ...
    def sync_model_parties_by_tier(self, party_ids, data):
        """
        Send global model to a list of parties by constructinga message
        with MessageType as `SYNC` and packaging the model as payload
        ...
        message = Message(MessageType.SYNC_MODEL_BY_TIER.value, data={'payload': data})
        id_request = message.get_header()['id_request']
